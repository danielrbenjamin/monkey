...
// ── Export: Copy Markdown (text only) ─────────────────────────────────────

document.getElementById('copyMdBtn').addEventListener('click', async () => {
  const displayTitle = guideTitle.textContent.trim() || 'My Guide';
  const lines = [`# ${displayTitle}`, '', `*${steps.length} steps · Generated by Monkey*`, ''];
  steps.forEach((s, i) => {
  ...
  try {
    ...
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a');
    a.href = url; a.download = `${slugify(displayTitle)}.md`; a.click();
    URL.revokeObjectURL(url);
...
document.getElementById('exportMdBtn').addEventListener('click', async () => {
  const title = slugify(guideTitle.textContent.trim() || 'my-guide');
  const displayTitle = guideTitle.textContent.trim() || 'My Guide';
  ...
  steps.forEach((s, i) => {
    const n = i + 1;
    lines.push(`## Step ${n}: ${s.description || ''}`, '');
    if (s.url) lines.push(`**Page:** ${s.url}`, '');

    if (s.screenshot) {
      // Detect format from data URL prefix (jpeg or png)
      const isJpeg = s.screenshot.startsWith('data:image/jpeg');
      const ext    = isJpeg ? 'jpg' : 'png';
      const fname  = `step-${String(n).padStart(2, '0')}.${ext}`;

      // Strip the data URL header and add raw base64 to the zip
      const b64 = s.screenshot.replace(/^data:image\/(jpeg|png);base64,/, '');
      imgFolder.file(fname, b64, { base64: true });

      lines.push(`![Step ${n}](images/${fname})`, '');
    }
  ...
  a.click();
  URL.revokeObjectURL(url);
  showToast('Markdown + images downloaded');
});
